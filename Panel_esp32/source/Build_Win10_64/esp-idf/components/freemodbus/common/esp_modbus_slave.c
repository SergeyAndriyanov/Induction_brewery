...
static mb_descr_entry_t *mbc_slave_find_reg_descriptor(mb_param_type_t type, uint16_t addr, size_t regs)
{
    ...
    for (it = LIST_FIRST(&mbs_opts->mbs_area_descriptors[type]); it != NULL; it = LIST_NEXT(it, entries))
    {
        ...
        if ((addr >= it->start_offset) && (it->p_data) && (regs >= 1) && ((addr + regs) <= (it->start_offset + reg_size + 1)) && (reg_size >= 1))
        {
            return it;
        }
    }
...
{
    esp_err_t error = ESP_OK;
    MB_SLAVE_CHECK((slave_interface_ptr != NULL), ESP_ERR_INVALID_STATE, "Slave interface is not correctly initialized.");
    ...
    else
    {
        ...
        mb_descr_entry_t *it;
        it = mbc_slave_find_reg_descriptor(descr_data.type, descr_data.start_offset, 1);
        MB_SLAVE_CHECK((it == NULL), ESP_ERR_INVALID_ARG, "mb incorrect descriptor or already defined.");
        mb_descr_entry_t *new_descr = (mb_descr_entry_t *)heap_caps_malloc(sizeof(mb_descr_entry_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
...
{
    esp_err_t error = ESP_OK;
    mb_register_area_descriptor_t descr_data = {0};
    ...
    case 0:
    {
        descr_data.type = MB_PARAM_HOLDING;
        descr_data.start_offset = MB_REG_HOLDING_START_AREA0;
        descr_data.address = (void *)&holding_reg_params.data_fl[0];
        descr_data.size = sizeof(holding_reg_params.data_fl)-4;
        break;
    }
    case 1:
    {
        descr_data.type = MB_PARAM_HOLDING;
        descr_data.start_offset = MB_REG_HOLDING_START_AREA1;
        descr_data.address = (void *)&holding_reg_params.data_ui[0];
        descr_data.size = sizeof(holding_reg_params.data_ui)-4;
        break;
    }
    case 2:
    {
        descr_data.type = MB_PARAM_INPUT;
        descr_data.start_offset = MB_REG_INPUT_START_AREA0;
        descr_data.address = (void *)&input_reg_params.data_fl[0];
        descr_data.size = sizeof(input_reg_params_t);
        break;
    }
    case 3:
    {
        descr_data.type = MB_PARAM_COIL;
        descr_data.start_offset = MB_REG_COILS_START;
        descr_data.address = (void *)&coil_reg_params;
        descr_data.size = sizeof(coil_reg_params);
        break;
    }
    case 4:
    {
        descr_data.type = MB_PARAM_DISCRETE;
        descr_data.start_offset = MB_REG_DISCRETE_INPUT_START;
        descr_data.address = (void *)&discrete_reg_params;
        descr_data.size = sizeof(discrete_reg_params);
        break;
    }
    default:
    {
        ...
        descr_data.start_offset = MB_REG_HOLDING_START_AREA0;
        descr_data.address = (void *)&holding_reg_params.data_fl[0];
        descr_data.size = sizeof(holding_reg_params_t);
        break;
    }
    }
...
void setup_reg_data(void)
{
    ...
    for (int i = 0; i < 63; i++)
    {
        holding_reg_params.data_fl[i] = i * 1.1;
        input_reg_params.data_fl[i] = i * 1.2;
        holding_reg_params.data_ui[i] = i;
        input_reg_params.data_ui[i] = i;
    }
}
//