...
esp_err_t i2s_zero_dma_buffer(i2s_port_t i2s_num)
{
    ...
    if (p_i2s_obj[i2s_num]->tx && p_i2s_obj[i2s_num]->tx->buf != NULL && p_i2s_obj[i2s_num]->tx->buf_size != 0) {
        ...
        if (bytes_left) {
            ...
            i2s_write(i2s_num, (void *)&zero_bytes, bytes_left, &bytes_written, portMAX_DELAY,0);
        }
        for (int i = 0; i < p_i2s_obj[i2s_num]->dma_buf_count; i++) {
...
esp_err_t i2s_write(i2s_port_t i2s_num, const void *src, size_t size, size_t *bytes_written, TickType_t ticks_to_wait,uint8_t* abr_datasend)
{
...
#endif
    src_byte = (char *)src;
    while (size > 0) {if(*abr_datasend==1){break;}
