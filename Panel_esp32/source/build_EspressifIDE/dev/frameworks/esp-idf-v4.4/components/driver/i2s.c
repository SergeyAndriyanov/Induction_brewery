...
esp_err_t i2s_zero_dma_buffer(i2s_port_t i2s_num)
{
    ...
    if (p_i2s[i2s_num]->tx && p_i2s[i2s_num]->tx->buf != NULL && p_i2s[i2s_num]->tx->buf_size != 0) {
        ...
        if (bytes_left) {
            ...
            i2s_write(i2s_num, (void *)&zero_bytes, bytes_left, &bytes_written, portMAX_DELAY,&tem);
        }
        for (int i = 0; i < p_i2s[i2s_num]->dma_buf_count; i++) {
...
esp_err_t i2s_driver_install(i2s_port_t i2s_num, const i2s_config_t *i2s_config, int queue_size, void *i2s_queue)
{
    ...
    if (ret != ESP_OK) {
        ...
        ESP_LOGE(TAG, "register I2S object to platform failed %i" ,ret);
 ...
 *     - ESP_ERR_INVALID_ARG  Parameter error
 */
esp_err_t i2s_write(i2s_port_t i2s_num, const void *src, size_t size, size_t *bytes_written, TickType_t ticks_to_wait,char* abrt)
{
